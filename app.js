(() => {
  ...
  async function loadReviews(){const root=$('reviewSessions');if(!root)return;try{const p=await api('listReviewSessions'),rows=(p.reviewSessions||[]).slice().reverse();root.innerHTML=`<h3>Review sessions</h3><div class="review-list">${rows.length?rows.map(r=>`<div class="review-session"><span><strong>${esc(r.Title)}</strong><small>${esc(r.Status)} · ${esc(r.ReviewDate)}</small></span><span class="pill">${esc(r.CreatedByUserID===state.user.userId?'You':'Shared')}</span></div>`).join(''):`<div class="review-session"><span><strong>No sessions yet</strong><small>No review sessions yet.</small></span></div>`}</div>`;}catch(e){root.innerHTML=`<h3>Review sessions</h3><p>${esc(e.message)}</p>`;}}
  async function createReview(){const b=$('newReviewBtn');busy(b,true,'Creating…');try{await api('createReviewSession',{});toast('Review created');await loadReviews();}catch(e){toast('Could not create review',e.message,'error');}finally{busy(b,false);}}
